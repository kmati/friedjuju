/*
 * This node.js script is used to build the j2f.
 */
var fs = require('fs'),
	path = require('path'),
...
function createBuilder() {
	...
	function copyReleaseToExamples(ctxt, callback) {
		var exampleBundlePath =  path.join(__dirname, '../examples/public/j2f-' + version + '.js');
	...
	function combineSrc(ctxt, callback) {
		var bundlePath =  path.join(ctxt.binDir, 'j2f-' + version + '.js');
	...
	function executeServices(target, services, callback) {
		var ctxt = {
			srcDir: path.join(__dirname, '../src/json-to-function'),
			binDir: path.join(__dirname, '..', 'bin', target)
		};
		ctxt.srcMainPath = path.join(ctxt.srcDir, 'j2f.js');
...
console.log('****************************************');
console.log('*                                      *');
console.log('*    build-j2f: The builder for j2f    *');
